...
export function Sidebar({ aiLive, label, sub, user }: { aiLive: boolean; label: string; sub: string; user: { name: string | null; email: string } }) {
    ...
    <aside className="fixed inset-y-0 left-0 z-40 flex w-[248px] flex-col p-3">
      <div className="glass flex h-full flex-col p-3">
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex items-center gap-2 rounded-2xl bg-surface/70 p-2.5">
          ...
          {/* Signed-in user + sign out (kept from your build) */}
          <div className="flex items-center gap-2.5 rounded-2xl bg-surface/70 p-2.5">
            <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[color:var(--accent-dim)] text-xs font-bold text-fg-dim">{initials}</div>
            ...
            <button
              ...
              aria-label="Sign out"
              title="Sign out"
              className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-fg-dim transition hover:bg-surface hover:text-fg disabled:opacity-50"
            >

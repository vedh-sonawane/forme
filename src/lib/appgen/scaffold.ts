...
`,
  ...
  // Design tokens from the project's FORME design system → the app looks on-brand.
  files.push({
...
:root {
  ...
  --r-sm: ${system.radius.sm};
  --r-md: ${system.radius.md};
  --r-lg: ${system.radius.lg};
  --sh-md: ${system.shadow.md};
}
...
h1, h2, h3, h4 { font-family: var(--font-heading), var(--font-body), sans-serif; letter-spacing: -0.02em; }

@layer components {
  .card { @apply rounded-2xl border bg-surface p-5; box-shadow: var(--sh-md); }
  .btn { @apply inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition disabled:opacity-50 disabled:pointer-events-none; }
  .btn-primary { @apply btn; background: var(--primary); color: var(--primary-text); }
  .btn-primary:hover { filter: brightness(1.08); }
  .btn-ghost { @apply btn border bg-surface text-fg; }
  .btn-ghost:hover { background: var(--surface-alt); }
  .input { @apply w-full rounded-xl border bg-surface px-3.5 py-2.5 text-sm outline-none transition; }
      ...
      <body>
        <header className="sticky top-0 z-50 border-b bg-bg/80 backdrop-blur">
          <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
            <Link href="/" className="font-display text-lg font-bold tracking-tight">${appName}</Link>
...
${navJsx}
        ...
        <footer className="border-t">
          <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-8 text-sm text-muted">
            <span>© {new Date().getFullYear()} ${appName}</span>
            <span>Built with FORME</span>
          </div>
        </footer>
      </body>
    </html>
